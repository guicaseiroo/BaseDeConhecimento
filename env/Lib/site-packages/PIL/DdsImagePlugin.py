...
DDS_HEADER_FLAGS_TEXTURE = (
    DDSD.CAPS | DDSD.HEIGHT | DDSD.WIDTH | DDSD.PIXELFORMAT
)
...
class DdsImageFile(ImageFile.ImageFile):
    ...
    def _open(self) -> None:
        ...
        # pixel format
        pfsize, pfflags, fourcc, bitcount = struct.unpack(
            '<4I', header.read(16)
        )
        n = 0
        rawmode = None
        ...
        if n:
            ...
            self.tile = [
                ImageFile._Tile('raw', extents, 0, rawmode or self.mode)
            ]
    ...
    def decode(self, buffer: bytes) -> tuple[int, int]:
        ...
        while len(data) < dest_length:
            ...
            for i, mask in enumerate(masks):
                ...
                data += o8(
                    int(
                        ((masked_value >> mask_offsets[i]) / mask_totals[i])
                        * 255
                    )
                )
