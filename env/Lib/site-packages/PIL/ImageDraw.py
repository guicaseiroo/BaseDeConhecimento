...
from typing import (TYPE_CHECKING, AnyStr, Callable, List, Sequence, Tuple,
                    Union, cast)
        ...
        if ink is not None:
            ...
            if joint == 'curve' and width > 4:
                ...
                for i in range(1, len(points) - 1):
                    ...
                    angles = [
                        math.degrees(
                            math.atan2(end[0] - start[0], start[1] - end[1])
                        )
                        % 360
                        for start, end in (
                            (points[i - 1], point),
        ...
        if fill_ink is not None:
            ...
            if full_x:
                self.draw.draw_rectangle(
                    (x0, y0 + r + 1, x1, y1 - r - 1), fill_ink, 1
                )
            else:
                self.draw.draw_rectangle(
                    (x0 + r + 1, y0, x1 - r - 1, y1), fill_ink, 1
                )
    ...
    def text(
        ...
        **kwargs,
        ...
        def draw_text(ink, stroke_width=0, stroke_offset=None) -> None:
            ...
            if stroke_offset:
                coord = [
                    coord[0] + stroke_offset[0],
                    coord[1] + stroke_offset[1],
                ]
            if mode == 'RGBA':
        ...
        if ink is not None:
            ...
            if stroke_width:
                stroke_ink = (
                    getink(stroke_fill) if stroke_fill is not None else ink
                )
...
def _compute_regular_polygon_vertices(
    ...
    elif len(bounding_circle) == 2 and isinstance(
        bounding_circle[0], (list, tuple)
    ):
        if not all(
    ...
    def _apply_rotation(
        point: list[float], degrees: float
    ) -> tuple[float, float]:
        return (
            round(
