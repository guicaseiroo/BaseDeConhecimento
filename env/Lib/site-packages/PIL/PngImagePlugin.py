...
class PngInfo:
    ...
    def add_itxt(
        ...
        if zip:
            ...
            self.add(
                b'iTXt', key + b'\0\0\0' + lang + b'\0' + tkey + b'\0' + value
            )
...
class PngImageFile(ImageFile.ImageFile):
    ...
    def _open(self) -> None:
        ...
        self.private_chunks: list[
            tuple[bytes, bytes] | tuple[bytes, bytes, bool]
        ] = []
    ...
    def _getexif(self) -> dict[str, Any] | None:
        if 'exif' not in self.info:
            self.load()
        if (
            'exif' not in self.info
            and 'Raw profile type exif' not in self.info
        ):
            return None
...
def _write_multiple_frames(
    ...
    disposal = im.encoderinfo.get(
        'disposal', im.info.get('disposal', Disposal.OP_NONE)
    )
    ...
    for im_seq in chain:
        for im_frame in ImageSequence.Iterator(im_seq):
            ...
            if im_frames:
                ...
                if (
                    prev_disposal == Disposal.OP_PREVIOUS
                    and len(im_frames) < 2
                ):
                ...
                if (
                    ...
                    previous['encoderinfo']['duration'] += encoderinfo[
                        'duration'
                    ]
                    continue
            else:
                bbox = None
            im_frames.append(
                {'im': im_frame, 'bbox': bbox, 'encoderinfo': encoderinfo}
            )
    ...
    if default_image:
        if im.mode != mode:
            im = im.convert(mode)
        ImageFile._save(
            im, _idat(fp, chunk), [('zip', (0, 0) + im.size, 0, rawmode)]
        )
...
def _save(im, fp, filename, chunk=putchunk, save_all=False):
    ...
    if save_all:
        ...
        for mode in ('RGBA', 'RGB', 'P'):
            ...
            mode = modes.pop()
        size = tuple(
            max(frame_size[i] for frame_size in sizes) for i in range(2)
        )
    else:
        size = im.size
        mode = im.mode
