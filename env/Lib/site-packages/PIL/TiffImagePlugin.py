...
class ImageFileDirectory_v2(_IFDv2Base):
    ...
    _load_dispatch: dict[
        int, Callable[[ImageFileDirectory_v2, bytes, bool], Any]
    ] = {}
    ...
    def __init__(
        ...
        self.reset()
        (self.next,) = (
            self._unpack('Q', ifh[8:])
            if self._bigtiff
            else self._unpack('L', ifh[4:])
        )
    ...
    def load_rational(self, data, legacy_api=True):
        ...
        return tuple(
            combine(num, denom) for num, denom in zip(vals[::2], vals[1::2])
        )

    @_register_writer(5)
    def write_rational(self, *values):
        return b''.join(
            self._pack('2L', *_limit_rational(frac, 2**32 - 1))
            for frac in values
        )
    ...
    def write_signed_rational(self, *values):
        return b''.join(
            self._pack(
                '2l', *_limit_signed_rational(frac, 2**31 - 1, -(2**31))
            )
            for frac in values
        )
        ...
        try:
            ...
            for i in range(tag_count):
                ...
                if size > (8 if self._bigtiff else 4):
                    here = fp.tell()
                    (offset,) = self._unpack(
                        'Q' if self._bigtiff else 'L', data
                    )
    ...
    def tobytes(self, offset=0):
        ...
        for tag, value in sorted(self._tags_v2.items()):
            ...
            if len(data) <= 4:
                ...
                entries.append(
                    (tag, typ, count, self._pack('L', offset), data)
                )
        ...
        for tag, typ, count, value, data in entries:
            logger.debug(
                '%s %s %s %s %s', tag, typ, count, repr(value), repr(data)
            )
...
class TiffImageFile(ImageFile.ImageFile):
    ...
    def _setup(self):
        ...
        if (
            len(sample_format) > 1
            and max(sample_format) == min(sample_format) == 1
        ):
...
def _save(im, fp, filename):
    ...
    if ROWSPERSTRIP not in ifd:
        ...
        if libtiff:
            ...
            rows_per_strip = (
                1 if stride == 0 else min(im_strip_size // stride, h)
            )
    ...
    if libtiff:
        ...
        for tag, value in itertools.chain(ifd.items(), supplied_tags.items()):
            ...
            if tag not in TiffTags.LIBTIFF_CORE:
                if not getattr(
                    Image.core, 'libtiff_support_custom_tags', False
                ):
                    continue
