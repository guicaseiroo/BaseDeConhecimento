...
class PpmPlainDecoder(ImageFile.PyDecoder):
    ...
    def _decode_blocks(self, maxval: int) -> bytearray:
        ...
        total_bytes = (
            self.state.xsize * self.state.ysize * bands * out_byte_count
        )
        ...
        while len(data) < dest_length:
            ...
            for b in range(bands):
                value = (
                    pixels[b]
                    if in_byte_count == 1
                    else i16(pixels, b * in_byte_count)
                )
...
def _save(im: Image.Image, fp: IO[bytes], filename: str | bytes) -> None:
    ...
    ImageFile._save(
        im, fp, [('raw', (0, 0) + im.size, 0, (rawmode, 0, row_order))]
    )
...
Image.register_extensions(
    PpmImageFile.format, ['.pbm', '.pgm', '.ppm', '.pnm', '.pfm']
)
