...
class XrefTable:
    ...
    def write(self, f):
        keys = sorted(
            set(self.new_entries.keys()) | set(self.deleted_entries.keys())
        )
...
def pdf_repr(x):
    if x is True:
        ...
        return (
            b'(D:'
            + time.strftime('%Y%m%d%H%M%SZ', x).encode('us-ascii')
            + b')'
        )
...
class PdfParser:
    ...
    def __init__(
        self, filename=None, f=None, buf=None, start_offset=0, mode='rb'
    ):
        if buf and f:
    ...
    def write_catalog(self) -> IndirectReference:
        ...
        self.rewrite_pages()
        self.write_obj(
            self.root_ref, Type=PdfName(b'Catalog'), Pages=self.pages_ref
        )
        self.write_obj(
            self.pages_ref,
    ...
    newline_only = rb'[\r\n]+'
    newline = (
        whitespace_optional_no_nl + newline_only + whitespace_optional_no_nl
    )
    ...
    re_dict_end = re.compile(
        whitespace_optional + rb'>>' + whitespace_optional
    )
    ...
    def interpret_trailer(cls, trailer_data):
        ...
        check_format_condition(
            b'Root' in trailer
            and isinstance(trailer[b'Root'], IndirectReference),
    ...
    def interpret_name(cls, raw, as_text=False):
        ...
        for m in cls.re_hashes_in_name.finditer(raw):
            if m.group(3):
                name += m.group(1) + bytearray.fromhex(
                    m.group(3).decode('us-ascii')
                )
            else:
                name += m.group(1)
        if as_text:
    ...
    re_null = re.compile(
        whitespace_optional + rb'null(?=' + delimiter_or_ws + rb')'
    )
    re_true = re.compile(
        whitespace_optional + rb'true(?=' + delimiter_or_ws + rb')'
    )
    re_false = re.compile(
        whitespace_optional + rb'false(?=' + delimiter_or_ws + rb')'
    )
    re_int = re.compile(
