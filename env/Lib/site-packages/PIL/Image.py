...
from typing import (IO, TYPE_CHECKING, Any, Literal, Protocol, Sequence, Tuple,
                    cast)

# VERSION was removed in Pillow 6.0.0.
# PILLOW_VERSION was removed in Pillow 9.0.0.
# Use __version__ instead.
from . import (ExifTags, ImageMode, TiffTags, UnidentifiedImageError,
               __version__, _plugins)
...
class Image:
    ...
    * :py:func:`~PIL.Image.frombytes`
        ...
        try:
            ...
            if not isinstance(e, (MemoryError, RecursionError)):
                try:
                    ...
                    if parse_version(numpy.__version__) < parse_version(
                        '1.23'
                    ):
                        warnings.warn(str(e))
            raise
    ...
    def convert(
        ...
        if matrix:
            ...
            if has_transparency and self.im.bands == 3:
                ...
                def convert_transparency(
                    ...
                    value = (
                        m[0] * v[0] + m[1] * v[1] + m[2] * v[2] + m[3] * 0.5
                    )
        ...
        if has_transparency:
            if (
                self.mode in ('1', 'L', 'I', 'I;16') and mode in ('LA', 'RGBA')
            ) or (self.mode == 'RGB' and mode in ('La', 'LA', 'RGBa', 'RGBA')):
    ...
    def filter(
        self, filter: ImageFilter.Filter | type[ImageFilter.Filter]
    ) -> Image:
        """
    ...
    def getextrema(self) -> tuple[float, float] | tuple[tuple[int, int], ...]:
        ...
        if self.im.bands > 1:
            return tuple(
                self.im.getband(i).getextrema() for i in range(self.im.bands)
            )
    ...
    def putpixel(
        ...
        * :py:mod:`~PIL.ImageDraw`
        ...
        if (
            ...
            value = (
                (palette_index, alpha) if self.mode == 'PA' else palette_index
            )
    ...
    def remap_palette(self, dest_map, source_palette=None):
        ...
        m_im.im.putpalette(
            palette_mode, palette_mode + ';L', m_im.palette.tobytes()
        )
    ...
    def resize(
        ...
        if resample is None:
            ...
            resample = (
                Resampling.NEAREST if type_special else Resampling.BICUBIC
            )
    ...
    def thumbnail(
        ...
        def preserve_aspect_ratio() -> tuple[int, int] | None:
            def round_aspect(number, key):
                return max(
                    min(math.floor(number), math.ceil(number), key=key), 1
                )
        ...
        if reducing_gap is not None:
            ...
            res = self.draft(
                None,
                (int(size[0] * reducing_gap), int(size[1] * reducing_gap)),
            )
            if res is not None:
                box = res[1]
        if box is None:
        ...
        if self.size != final_size:
            im = self.resize(
                final_size, resample, box=box, reducing_gap=reducing_gap
            )
        ...
        if init():
            im = _open_core(
                ...
                tuple(
                    ...
                    if format not in checked_formats
                ),
            )
...
class Exif(_ExifBase):
    ...
    def _get_merged_dict(self):
        ...
        if ExifTags.IFD.Exif in self:
            ifd = self._get_ifd_dict(
                self[ExifTags.IFD.Exif], ExifTags.IFD.Exif
            )
            if ifd:
        ...
        for tag, value in self.items():
            if tag in [
                ...
                if (
                    ...
                    value = value.copy()
                    value[ExifTags.IFD.Interop] = self.get_ifd(
            ...
            if tag == ExifTags.IFD.IFD1:
                ...
                if tag == ExifTags.IFD.Makernote:
                    ...
                    if tag_data[:8] == b'FUJIFILM':
                        ...
                        makernote = {}
                        for i in range(
                            0, struct.unpack('<H', ifd_data[:2])[0]
                        ):
                        ...
                        makernote = {}
                        for i in range(
                            0, struct.unpack('>H', tag_data[:2])[0]
                        ):
                            ifd_tag, typ, count, data = struct.unpack(
                                '>HHL4s',
                                tag_data[i * 12 + 2 : (i + 1) * 12 + 2],
                            )
                            if ifd_tag == 0x1101:
                                # CameraInfo
                                (offset,) = struct.unpack('>L', data)
                                ...
                                makernote = {
                                    0x1101: dict(self._fixup_dict(camerainfo))
                                }
    ...
    def __getitem__(self, tag):
        if (
            ...
            and tag not in self._data
            and tag in self._info
        ):
