...
from ..forms import (ExampleModelForm, ExampleModelOverriddenWidgetForm,
...
class ModelFieldAndFormFieldTestCase(TestCase):
    ...
    def test_upload_model_form_with_overridden_widget_contains_expected_formfield(
        self,
    ):
        ...
        self.assertEqual(
            widget.config.get('extraPlugins'), 'someplugin,anotherplugin'
        )
    ...
    def test_admin_model_forms_contain_expected_formfields(self):
        ...
        response = self.client.get(
            reverse('admin:demo_application_examplemodel_add')
        )
        self.assertEqual(response.status_code, HTTPStatus.OK)
